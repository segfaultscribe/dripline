...
import type { AdminService } from "./ServiceFactory";

import { DuplicateExternalUserError, UserNotFoundError } from "./ServiceFactory";
  ...
  return new Elysia({ prefix: '/admin' })
    ...
    .post(
      ...
      async ({ body, set }) => {
        ...
        try {
          const creationResult = adminService.createEndUser(externalUserId, dailyRequestLimit);
    ...
    .post(
      ...
      async ({ params: { id }, set, body }) => {
        ...
        try {
          const keyCreationResult = adminService.createUserApiKey(id, keyName);
    ...
    .get(
      ...
      ({params: {id}, set}) => {
        try{
          const usage = adminService.getUserUsageSummary(id);
          return usage;
        } catch (err: unknown) {
          if (err instanceof UserNotFoundError) {
            set.status = 404;
    ...
    .get(
      ...
      ({set}) => {
        try{
          const result = adminService.getUsageSummary()
          return result;
        } catch(err: unknown) {
          console.error(`Error retrieving usage summary: ${err}`);
          set.status = 500;
